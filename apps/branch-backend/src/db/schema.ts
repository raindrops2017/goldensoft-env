...
export const deliveryCustomers = sqliteTable('delivery_customers', {
  ...
  averageTicket: real('average_ticket').notNull().default(0),
  cloudSyncId: text('cloud_sync_id'),
  createdAt: text('created_at').notNull().default(sql`(datetime('now'))`),
  updatedAt: text('updated_at').notNull().default(sql`(datetime('now'))`),  
});
...
export const deliveryAddresses = sqliteTable('delivery_addresses', {
  id: text('id').primaryKey(),
  deliveryCustomerId: text('delivery_customer_id').notNull().references(() => deliveryCustomers.id, { onDelete: 'cascade' }),
...
export const deliveryPhones = sqliteTable('delivery_phones', {
  id: text('id').primaryKey(),
  deliveryCustomerId: text('delivery_customer_id').notNull().references(() => deliveryCustomers.id, { onDelete: 'cascade' }),
