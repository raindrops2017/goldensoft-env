...
export function NumpadPopup({
  ...
  return createPortal(
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2">
        {keys.flat().map((key) => (
          <button
            ...
            className={`
              h-12 rounded-xl flex items-center justify-center text-lg font-bold transition-all active:scale-95
              ${
                key === "C"
      ...
      <button
        onClick={onClose}
        className="w-full mt-4 py-3 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-bold shadow-lg shadow-brand-500/25 transition-all active:scale-[0.98]"
      >
        Confirm
      </button>
    </div>,
